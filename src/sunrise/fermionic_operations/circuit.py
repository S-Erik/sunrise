...
class FCircuit:
    ...
    def add_controls(self, control, inpl: Optional[bool] = False) -> Optional[FCircuit]: 
        pass 
        ...
        if molecule is not None:
            ...
            for gate in U.gates:
                if gate.name == 'UR':
                    ...
                    res += molecule.make_excitation_gate(indices=gate.indices[1],angle=gate.variables)
                elif gate.name in ['UC', 'FermionicExcitation', 'GenericFermionic']:
                    res += molecule.make_excitation_gate(indices=gate.indices[0],angle=gate.variables)
                elif gate.name == 'Ph':
                    res +=  Phase(target=[0][0],angle=gate.variables)
                else:
